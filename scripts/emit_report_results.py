...
def tc_macroword_value(value):
    """Rendered word count of an emitted macro value, by texcount's own rule.

    texcount counts 0 words for macros it does not know, so every emitted
    macro also gets a ``%TC:macroword`` directive declaring its rendered word
    value; the official count must run ``texcount -merge`` (directives are
    file-scoped without it). Emitted values are only ever a bare number
    ("0.766"), a percentage ("13.2\\%"), a value-with-unit ("-19.5\\,pp"),
    or a placeholder em-dash: texcount counts a number or number-with-% as
    one word and ``\\,`` as a word separator, so the rule reduces to
    placeholder = 0, value+unit = 2, otherwise 1.
    """
    text = str(value)
    if "\\ResultPlaceholder" in text or "\\textemdash" in text:
        return 0
...
def render_macros(macros):
    ...
    lines += [
        "",
        "% texcount word-value directives: one per macro above, so the official",
        "% count (texcount -merge ...) includes each macro's rendered words.",
        "% File-scoped without -merge; the hand-maintained display-name",
        "% directives and the count command live in construction/macros.tex.",
        r"%TC:macroword \ResultPlaceholder 0",
    ]
    for name, value in macros.items():
        lines.append(rf"%TC:macroword \{name} {tc_macroword_value(value)}")
    return "\n".join(lines) + "\n"
...
M4_MATCH_REL_TOL = 0.1  # mirrors run_real_eval.py's R~-bracket monitor
...
# figure's bars are (make_real_figures.fig_bimodal_enrichment), for Joint MAP
...
# run_real_probes.py) re-solves the resolved sweep rows with the restart RNG
