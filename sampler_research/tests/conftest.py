...
`needs_real`/`needs_toy`/`needs_phase4_run` skip markers that were previously
re-declared in each test module. This is a pure test-support refactor: the
...
resolves. `REPO_ROOT` is `parents[2]` of this file, which lives in the same
