...
\newcommand{\summarywordcount}{975}
...
\comparisonNInits{} start dates spanning the seasons of 2023, outside the model's
training period.
...
conditions where the sampler matters most grow with range. Figure~\ref{fig:sum-maps} shows the
...
\begin{figure}[t]
  ...
  Atlantic and Europe, in standardised units. Sampling each location separately is
  noisy (top left), whereas Joint MAP (top right) and the smoothed baseline (bottom
  left) are equally smooth by construction, and the corresponding ERA5 field (bottom
  right) shows the weather that occurred.}
  \label{fig:sum-maps}
\end{figure}
