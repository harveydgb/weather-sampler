...
\newcommand{\summarywordcount}{990}
...
% executive-summary/ reports "Words in text: 980" plus "Words in headers: 10"; the
% 990 shown INCLUDES the section headings (980 + 10), the conservative reading, and
...
planning, and with recent years repeatedly breaking temperature records, this means preparing for
droughts and heatwaves depends not only on the most likely outcome but on how
confident that prediction is. Operational ensembles traditionally estimate this uncertainty 
through repeated physics-based simulations that vary initial conditions and model behaviour.
Machine-learning models offer a newer route, learning from decades of past atmospheric data 
without directly simulating the physics. Some, such as GenCast \cite{price2024gencast},
generate whole ensembles of fields. The WeatherGenerator \cite{weathergenerator}, a foundation weather model
developed at ECMWF is related to AtmoRep \cite{lessig2023atmorep} and Aurora
\cite{bodnar2024aurora}. Its branch instead outputs a Gaussian mixture at every
grid location, a blend of four weighted bell-shaped components. Each mixture can 
therefore represent uncertainty and hold several distinct plausible values at once.
However, due to computational constraints each location's mixture is produced on
